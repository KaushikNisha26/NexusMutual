...
contract claims{
    ...
    {   
        ...
        if(status>6)
            ...
            close=1;
        else if(date_upd+c1.maxtime()<=now) //max voting period=48hrs
            close=1;
        else if(date_upd+ c1.mintime()>=now) //min voting period=12 hrs
            close=0;
    ...
    function changePendingClaimStart() onlyInternal
    {
        ...
        //while(pendingClaim_start<allClaims.length)
